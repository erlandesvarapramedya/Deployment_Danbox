...
with col11:
    ...
    st.write('Anda memilih:', besar2)
    
with col12:
    st.subheader("Gelas Ukur")
    image = Image.open('gambar/pia.jpg')
    st.image(image, width=150)
    import streamlit as st
    numberF = st.number_input('Jumlah Gelas Ukur',0)
    ukuran = st.selectbox(
        'Pilih Ukuran Gelas Ukur(mL)',
            ('10', ' 25','50','100','150','200','250'))
    st.write('Anda memilih:', ukuran)

with st.form(key='my_form'):
    lab=st.form_submit_button("Total")
if lab:
    import pandas as pd
    hargaA=3000*numberA
    hargaB=5000*numberB
    hargaC=15000*numberC
    hargaD=50000*numberD
    hargaE=30000*numberE
    hargaF=30000*numberF
    data = [['Sarung Tangan',numberA,hargaA],['Serbet',numberB,hargaB],['Tabung Reaksi',numberC,hargaC],['Gelas Piala',numberD,hargaD],['Tabung Ulir',numberE,hargaE],['Gelas Ukur',numberF,hargaF]]
...
hargaE=30000
hargaF=30000
hitung=(hargaA*numberA)+(hargaB*numberB)+(hargaC*numberC)+(hargaD*numberD)+(hargaE*numberE)+(hargaF*numberF)
if hitung<50000:
    st.write("total harga perlengkapan Rp.",hitung)
elif hitung>=50000:
    harga=hitung-0.1*hitung
    tulis=st.write("Diskon 10% total harga perlengkapan Rp.",harga)
...
if st.button('KIRIM'):
    st.success('Pesanan Diterima. Terimakasih Atas Pesanannya!')
